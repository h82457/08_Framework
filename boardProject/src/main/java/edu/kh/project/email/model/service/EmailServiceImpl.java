...
public class EmailServiceImpl implements EmailService{
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			// 1번 매개변수 : MimeMessage, 2번 매개변수 : 파일 전송 사용? true/false , 3번 매개변수 : 문자 인코딩 지정
			
			helper.setTo(email); // 받는 사람 이메일 지정
			helper.setSubject(subject); // 이메일 제목 지정
			helper.setText(loadHTML(authKey, htmlName), true);	// html 코드 해석 여부 true, innerHTML 해석 
			
			// CID(Content-ID)를 이용해 메일에 이미지를 첨부 != 파일 첨부 <- 이메일 내용에 사용할 이미지 첨부
			helper.addInline("logo", new ClassPathResource("static/images/logo.jpg"));
			
			// 로고 이미지를 메일 내용에 첨부, 사용하고 싶으면 "logo"라는 아이디를 작성
	...
	public String loadHTML(String authKey, String htmlName) {
		...
		Context context = new Context(); // 자바에서 타임리프 사용 가능하게 함
